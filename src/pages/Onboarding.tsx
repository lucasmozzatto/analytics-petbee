...
export default function Onboarding() {
  ...
  const biggestDrop = data?.steps.reduce<{ stepName: string; lost: number; stepNumber: number }>(
    (max, step, i) => {
      if (i === 0) return max;
      const prev = data.steps[i - 1];
      const lost = prev.users - step.users;
      return lost > max.lost ? { stepName: step.stepName, lost, stepNumber: step.stepNumber } : max;
    },
  ...
  const kpiCards = [
    ...
    {
      label: 'MAIOR PERDA',
      value: biggestDrop && biggestDrop.lost > 0
        ? getStepLabel(biggestDrop.stepNumber, biggestDrop.stepName)
        : '—',
      subtitle: biggestDrop && biggestDrop.lost > 0
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold" style={{ color: 'var(--text)' }}>Onboarding</h1>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr style={{ borderBottom: '1px solid var(--border)' }}>
                      {['#', 'Etapa', 'Fluxo', 'Usuários', 'Taxa Total', 'Drop Step', 'Perdidos'].map((h) => (
                        <th
                          key={h}
                          className={`px-4 py-3 text-[10px] font-semibold tracking-wider ${
                            ['Usuários', 'Taxa Total', 'Drop Step', 'Perdidos'].includes(h) ? 'text-right' : 'text-left'
                          }`}
                  ...
                  <tbody>
                    {data.steps.map((step, i) => {
                      ...
                      const flowChanged = prevConfig && config
                        && prevConfig.flow !== config.flow
                        && prevConfig.flow !== 'shared'
                        && config.flow !== 'shared';

                      const lost = prevStep && !flowChanged ? prevStep.users - step.users : 0;
                        ...
                        <>
                          ...
                          <tr
                            ...
                            style={{
                              ...
                              backgroundColor: step.stepRate < 70 && i > 0 && !flowChanged
                                ? 'rgba(239, 68, 68, 0.04)'
                                : undefined,
                            }}
                          >
                            ...
                            <td className="px-4 py-2.5 text-xs" style={{ color: 'var(--text-dim)' }}>
                            ...
                            <td
                              ...
                              style={{
                                fontFamily: 'var(--mono)',
                                color: !flowChanged && step.stepRate < 70 && i > 0
                                  ? 'var(--red)'
                                  : 'var(--text-dim)',
                              }}
                            >
                              {i === 0 || flowChanged ? '—' : formatPercent(step.stepRate, 1)}
                            </td>
                            <td
                              className="px-4 py-2.5 text-xs text-right"
                              style={{
                                fontFamily: 'var(--mono)',
                                color: lost > 0 ? 'var(--red)' : 'var(--text-muted)',
                              }}
                            >
                              {i === 0 || flowChanged ? '—' : `-${formatNumber(lost)}`}
                            </td>
                          </tr>
                        </>
                      );
          ...
          {data.steps.length === 0 && (
            ...
            >
              ...
              <p className="text-xs mt-2" style={{ color: 'var(--text-muted)' }}>
                Verifique se o sync foi executado e se há eventos onboarding_step no GA4.
              </p>
            </div>
          )}
        </>
